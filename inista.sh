 #!/bin/bash                                                                                   
  ...
  install_proxy() {                                                                             
      ...
      [ "$ENABLE_CACHE" = "y" ] && echo -e "                                                    
  缓存：${GREEN}启用（${CACHE_DAYS}天，${CACHE_SIZE}GB）${NC}"                                  
  ...
  generate_nginx_config() {                                                                     
      ...
      if [ "$ENABLE_CACHE" = "y" ]; then                                                        
          ...
          mkdir -p "$CACHE_PATH"                                                                
          CACHE_CONFIG="proxy_cache_path ${CACHE_PATH} levels=1:2 keys_zone=tmdb_cache:50m      
  inactive=${CACHE_DAYS}d max_size=${CACHE_SIZE}g;"                                             
      fi                                                                                        
